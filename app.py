...
with tab2:
    ...
    if type_ == 'transfer':
        ...
        with st.form("add_transfer_form"):
            ...
            with col1:
                from_account_name = st.selectbox("From", account_df['name'])
            with col2:
                to_account_name = st.selectbox("To", account_df['name'])
            transfer_amount = st.number_input("Transferred Amount", min_value=0.0, step=1.0)
            ...
            if submitted:
                if from_account_name == to_account_name:
                    ...
                    from_id = int(account_df[account_df['name'] == from_account_name]['account_id'].iloc[0])
                    to_id = int(account_df[account_df['name'] == to_account_name]['account_id'].iloc[0])
                    db.insert_transfer(str(date), from_id, to_id, transfer_amount, note)
                    st.cache_data.clear()
        ...
        with st.form("add_transaction_form"):
            ...
            if submitted:
                account_id = int(account_df[account_df['name'] == account_name]['account_id'].iloc[0])
